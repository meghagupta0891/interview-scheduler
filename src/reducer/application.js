...
export default function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case SET_APPLICATION_DATA:
      const days = action.value[0] ? action.value[0].data:[];
      const appointments = action.value[1] ? action.value[1].data:{};
      const interviewers = action.value[2] ? action.value[2].data: {};
